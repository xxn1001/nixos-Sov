...
}:
{
  ...
  xdg.configFile."go-musicfox/config.toml".text =
    ...
    ''
      ...
      downloadDir = ""
      # 文件名模板
      fileNameTpl = "{{.SongName}}-{{.ArtistName}}.{{.SongType}}"

      # 缓存配置
      [storage.cache]
      # 缓存目录
      # !!!注意!!! 如果使用mpd,mpd配置中的"music_directory"必须与musicDir一致
      dir = ""
      ...
      # engine = "beep"
      # 期望的歌曲音质，standard,higher,exhigh,lossless,hires
      songLevel = "higher"
      ...
      configFile = "/Users/anhoder/.mpd/mpd.conf"
      # tcp 或 unix
      network = "unix"
